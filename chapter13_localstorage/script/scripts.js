...
function renderTodos(todos) {
  ...
  todos.forEach((todo, index) => {
    ...
    // 기능과 관련된 부분이 없음 -> 즉, renderTodos() 내부에 button의 삭제 버튼을 구현해둬야 함.

    // 물론 우리가 배운대로 외부에 구현해놓고 여기서 호출만 해도 되는데 메커니즘을 설명하기 위해 여기에 작성하겠음.

    deleteBtn.onclick = () => removeTodo(index);  // 버튼을 클릭하면 removeIndex() 함수를 호출
    // argument로 index를 받음 -> 즉, 특정 요소 하나만 삭제하는 버튼이라고 볼 수 있음.
    // deleteBtn.addEventListener('click', removeTodo(index));와 동일어
    // deleteBtn.addEventListener('click', function() {
    //    삭제하는 구현부; 와 같은 방식으로는 이전에 작성해왔음    
    // })
...
function removeTodo(index) {
  ...
  // '배열에서만의 삭제'가 의미하는 바? -> 로컬스토리지에는 남아있음 -> 그렇다면 다시 읽어오게 된다면
