...
// 배열에 값 대입 형식 -> 해단 부분은 Java와 좀 차이가 있기 때문에 유심히 보기
...
// 이상의 코드에서 확인해봤을 때 '오렌지'가 '딸기'로 대체되었음을 확인할 수 있음. 즉, index를 한 칸씩 민다거나 하는건 아님.


// 이제 배열을 사용해서 실질적으로 로또 번호 당첨 생성기를 구현할 예정

// 1. LottoArray 배열을 선언하고 
// 2. 해당 배열 내에 1 ~ 45까지의 number 값을 넣는다.
// 3. .pop()해서 하나씩 뽑아서 출력하면 어느 정도 구현됨.
...
// 저 위에서 고려한 방식은 기본적으로 1~45까지의 int를 생성하는 방식이었는데, 이미 배열에 가 만들어둔 상태입니다.

// Math.floor(Math.random()) 관련한 코드를 어디다가 쓸거냐면 
// index number를 산출할 때 사용할거임.
...
// 이상의 코드를 사용했을 경우 배열에서의 특정 elemt가 num에 저장됨.

// 그렇다면 이걸 lottoArray에서 그 num과 일치하는 값을 삭제
...
// 여기에 반복문을 적용해서 6개의 숫자를 LottoNumbers에 대입할 예정임.
...
// script.js 코드 검증하는 부분들 작성 -> main은 script2.js 확인
